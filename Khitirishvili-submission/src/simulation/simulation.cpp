...
SystemStats Simulation::calculate_statistics()
{
    ...
    {
        std::shared_ptr<Process> process = p.second;

        for (std::shared_ptr<Thread> thread : process->threads)
        {

            switch (thread->priority)
            {
            case SYSTEM:
                system_stats.thread_counts[0] += 1;
            ...
            case INTERACTIVE:
                ...
                system_stats.thread_counts[1] += 1;
                break;
            case NORMAL:
                system_stats.thread_counts[2] += 1;
                ...
                break;
            case BATCH:
                system_stats.thread_counts[3] += 1;
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        {
            // Cast integers to floats before division
            system_stats.avg_thread_response_times[i] = static_cast<float>(system_stats.avg_thread_response_times[i]) / static_cast<float>(system_stats.thread_counts[i]);
            system_stats.avg_thread_turnaround_times[i] = static_cast<float>(system_stats.avg_thread_turnaround_times[i]) / static_cast<float>(system_stats.thread_counts[i]);
        }
    }

    system_stats.cpu_utilization = 100 * (static_cast<float>(system_stats.total_time) - static_cast<float>(system_stats.total_idle_time)) / static_cast<float>((system_stats.total_time));
    system_stats.cpu_efficiency = 100 * static_cast<float>(system_stats.total_service_time) / static_cast<float>(system_stats.total_time);
